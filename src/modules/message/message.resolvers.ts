...
export class MessageResolvers {
  ...
  async generateChatConversationWith(
    ...
    let existingSession =
      await this.messageService.findConversationUniqueByMembers(members);
  ...
  async generateChatConversationWithOrg(
    ...
    let existingSession =
      await this.messageService.findConversationBetweenUserAndOrg(
        user.id,
        chatWithOrgId,
        orgDetail.createdBy,
      );
    ...
    if (!existingSession) {
      existingSession =
        await this.messageService.createNewConversationBetweenUserAndOrg(
          user.id,
          {
            orgId: chatWithOrgId,
  ...
  async feedMyOrgConversations(
    ...
    const result = await this.messageService.findAllMyOrgConversations({
      offset,
      limit,
